...
float ImGui::ScaleRatioFromValueT(ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    // Linear slider
    return (float)((FLOATTYPE)((SIGNEDTYPE)v_clamped - (SIGNEDTYPE)v_min) / (FLOATTYPE)((SIGNEDTYPE)v_max - (SIGNEDTYPE)v_min));
}
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    else
    {
        ...
        else
        {
            ...
            // Not doing floating point math when we are at the end of range (clicked_t=1.0) avoids an
            // edgecase where multiplying this number by 1.0 would yield a different number due to floating
            // point imprecision. This becomes a problem at the end of very big range, for example when
            // v_min = UINT64_MAX >> 1 and v_max = 0.
            if (t < 1.0)
            {
                FLOATTYPE v_new_off_f = ((SIGNEDTYPE)v_max - (SIGNEDTYPE)v_min) * t;
                result = (SIGNEDTYPE)v_min + (SIGNEDTYPE)(v_new_off_f + (FLOATTYPE)(v_min > v_max ? -0.5 : 0.5));
            }
            else
            {
                result = (SIGNEDTYPE)v_min + ((SIGNEDTYPE)v_max - (SIGNEDTYPE)v_min);
            }
        }
    }
