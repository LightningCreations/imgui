...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)//, float size)
{
    ...
    if (hovered)
        window->DrawList->AddCircleFilled(center, ImMax(2.0f, g.FontSize * 0.5f + style.PointSizeRound), col, 12);

    float cross_extent = g.FontSize * 0.5f * 0.7071f - 1.0f;
    ImU32 cross_col = GetColorU32(ImGuiCol_Text);
    center -= ImVec2(0.5f, 0.5f);
    window->DrawList->AddLine(center + ImVec2(+cross_extent, +cross_extent), center + ImVec2(-cross_extent, -cross_extent), cross_col, style.PointSizeRound);
    window->DrawList->AddLine(center + ImVec2(+cross_extent, -cross_extent), center + ImVec2(-cross_extent, +cross_extent), cross_col, style.PointSizeRound);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos, ImGuiDockNode* dock_node)
{
    ...
    ImVec2 center = bb.GetCenter();
    if (hovered || held)
        window->DrawList->AddCircleFilled(center + off + ImVec2(0,-0.5f), g.FontSize * 0.5f + style.PointSizeRound, bg_col, 12);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    ImRect bb = bb_frame;
    bb.Expand(ImVec2(-ImClamp(IM_FLOOR((bb_frame_width - 2.0f * style.PointSize) * 0.5f), 0.0f, 3.0f * style.PointSize), -ImClamp(IM_FLOOR((bb_frame_height - 2.0f * style.PointSize) * 0.5f), 0.0f, 3.0f * style.PointSize)));
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
    ...
    ImVec2 border_size(g.Style.ChildBorderSize, g.Style.ChildBorderSize);
    if (border_col.w > 0.0f)
        bb.Max += border_size * 2;
    ItemSize(bb);
    if (!ItemAdd(bb, 0))
        return;
    ...
    if (border_col.w > 0.0f)
    {
        ...
        window->DrawList->AddImage(user_texture_id, bb.Min + border_size, bb.Max - border_size, uv0, uv1, GetColorU32(tint_col));
    }
    else
    {
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    const float radius = (square_sz - style.PointSizeRound) * 0.5f;
    ...
    if (active)
    {
        const float pad = ImMax(style.PointSizeRound, IM_FLOOR(square_sz / 6.0f));
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    float thickness_draw = style.PointSizeRound;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(g.StyleTemplate.FramePadding.x, g.StyleTemplate.WindowPadding.y));
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    const float grab_padding = IM_FLOOR(2.0f * style.PointSize);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        PushStyleVar(ImGuiStyleVar_ChildRounding, g.StyleTemplate.FrameRounding);
        PushStyleVar(ImGuiStyleVar_ChildBorderSize, g.StyleTemplate.FrameBorderSize);
        PushStyleVar(ImGuiStyleVar_WindowPadding, g.StyleTemplate.FramePadding);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            float bg_offy_up = is_multiline ? 0.0f : -style.PointSizeRound;    // FIXME: those offsets should be part of the style? they don't play so well with multi-line selection.
            float bg_offy_dn = is_multiline ? 0.0f : IM_FLOOR(2.0f * style.PointSize);
        ...
        if (render_cursor)
        {
            ...
            ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y - g.FontSize + 0.5f, cursor_screen_pos.x + style.PointSize, cursor_screen_pos.y - 1.5f * style.PointSize);
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(g.StyleTemplate.ItemSpacing.x * 2.0f, g.StyleTemplate.ItemSpacing.y));
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags, ImGuiDockNode* dock_node)
{
    ...
    const float y = tab_bar->BarRect.Max.y - style.PointSizeRound;
    if (dock_node != NULL)
    {
        ...
        window->DrawList->AddLine(ImVec2(separator_min_x, y), ImVec2(separator_max_x, y), col, style.PointSizeRound);
    }
    else
    {
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiColumnsFlags flags)
{
    ...
    columns->OffMaxX = ImMax(ImMin(max_1, max_2) - window->Pos.x, columns->OffMinX + style.PointSizeRound);
    ...
    for (int n = 0; n < columns_count; n++)
    {
        ...
        float clip_x2 = IM_ROUND(window->Pos.x + GetColumnOffset(n + 1) - style.PointSizeRound);
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiColumnsFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int n = 1; n < columns->Count; n++)
        {
            ...
            window->DrawList->AddLine(ImVec2(xi, y1 + style.PointSizeRound), ImVec2(xi, y2), col);
        }
